...
# Read forces directly, skipping first 2 samples per window
...
for d in dirs:
    ...
    with open(ff) as f:
        for line in f:
            ...
            count += 1
            if count <= 2: continue  # skip transient
...
fig.suptitle(f"Cosim forces (cleaned) — Wg0={GUST_W0:.0f} m/s, U∞={U_INF:.0f} m/s", fontweight="bold")
...
axes[0].plot(t, Cl, "C0", lw=0.6); axes[0].set_ylabel("CL [-]")
axes[0].legend(fontsize=7, loc="upper left")
axes[1].plot(t, Fy, "C2", lw=0.6); axes[1].set_ylabel("Fy [N]")
axes[2].plot(t, Mz, "C3", lw=0.6); axes[2].set_ylabel("Mz [N·m]")
axes[2].set_xlabel("Time [s]"); axes[2].set_xlim(0, 5)

fig.tight_layout()
Path("figures").mkdir(exist_ok=True)
fig.savefig("figures/gust_response_clean.png", dpi=150, bbox_inches="tight")
print("Saved → figures/gust_response_clean.png")
